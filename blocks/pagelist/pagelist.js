...
function renderCard(item) {
  ...
  li.className = 'pagelist-item';

  const link = document.createElement('a');
  link.className = 'pagelist-link';
  link.href = item.path;

  if (item.image) {
    const picture = createOptimizedPicture(item.image, item.title || '', false, [{ width: '750' }]);
    const media = document.createElement('div');
    media.className = 'pagelist-image';
    media.append(picture);
    link.append(media);
  }

  const bodyEl = document.createElement('div');
  bodyEl.className = 'pagelist-body';

  const title = document.createElement('h3');
  title.className = 'pagelist-title';
  title.textContent = item.title || item.path;
  bodyEl.append(title);

  if (item.description) {
    const desc = document.createElement('p');
    desc.className = 'pagelist-desc';
